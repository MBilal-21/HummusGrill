...
const ProfileComponent = () => {
  ...
  const handleProfile = (e) => {
    ...
    if (
      window.confirm(
        `${profileData.firstName} are you want sucessfully change data`
      )
    ) {
  ...
  const handlePassword = (e) => {
    ...
    if (udata.password !== passChange.oldPass) {
      setPassError("Old Password is In correct");
      setbgPassColr(warningColor);
    } else if (passChange.newPass !== passChange.comfirmPass) {
      setPassError("Password does not match");
      setbgPassColr(warningColor);
    } else if(passChange.newPass.length < 6) {
      setPassError("Minimum Password length should be 6");
      setbgPassColr(warningColor);
    } else if(passChange.newPass === passChange.oldPass) {
      setPassError("Password should be different from old password");
      setbgPassColr(warningColor);
    }else {
            ...
            {profileError && (
              <Col md={12}>
                <div
                  ...
                  style={{ background: bgProfileColr }}
                >
